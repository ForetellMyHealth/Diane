...
# 4. MSS ratio calculation
normal_mss <- tr_adjust %>% filter(state == '0') %>% dplyr::select(-sample,-state) %>% apply(2, function(x){mean(x^2)}) %>% as.data.frame()
cancer_mss <- tr_adjust %>% filter(state == '1') %>% dplyr::select(-sample,-state) %>% apply(2, function(x){mean(x^2)}) %>% as.data.frame()

theta <- (cancer_mss / normal_mss) %>% rownames_to_column('gene_name') %>% arrange(desc(.))
colnames(theta)[2] <- 'MSS Ratio'

# write.csv(theta, '../data_revised/GAPDH_scale_trainNDFGR_zerosum_MSS_ratio.csv')

normal_mss <- normal_mss %>% rownames_to_column('gene_name')
cancer_mss <- cancer_mss %>% rownames_to_column('gene_name')
colnames(normal_mss)[2] <- 'Normal MSS'
colnames(cancer_mss)[2] <- 'Cancer MSS'

mss_data <- inner_join(cancer_mss, normal_mss, by='gene_name') %>% inner_join(theta, by='gene_name')
mss_data_symbol <- inner_join(gene_data[1:2], mss_data, by=c('id'='gene_name')) %>% arrange(desc(`MSS Ratio`))
# write.csv(mse_data_symbol, '/nfs-data/ULMS/Diane/data_revised/GAPDH_scale_trainNDFGR_zerosum_MSS_cancer_normal_ratio_symbol.csv')
